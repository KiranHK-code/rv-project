...
export default function Dashboard({ apiBase }) {
  ...
  if (loading) {
    return <div className="panel"><p>Loading dashboard...</p></div>;
  }

  if (!dashData) {
    return <div className="panel"><p>No data available</p></div>;
  }
  ...
  return (
    <div className="panel">
      ...
      <div className="card">
        ...
        <div className="grid-2">
          {dashData.inventory.warehouseDetails.map(wh => (
            <div key={wh.id} style={{ padding: '16px', borderLeft: wh.lowStockAlert ? '4px solid #ff6b6b' : '4px solid #51cf66' }}>
              <h4>{wh.name}</h4>
              <p>Stock: <strong>{wh.stock.toLocaleString()}</strong> / {wh.capacity.toLocaleString()} units</p>
              <div style={{ marginTop: '8px', background: '#1a1a1a', height: '8px', borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{
              ...
              <p style={{ marginTop: '8px', fontSize: '12px', color: '#888' }}>{wh.utilization}% utilized</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="grid-5">
          {Object.entries(dashData.orders).map(([status, count]) => (
            <div key={status} style={{ textAlign: 'center', padding: '16px', background: '#1a1a1a', borderRadius: '6px' }}>
              ...
              <div style={{ fontSize: '12px', marginTop: '8px', textTransform: 'capitalize' }}>{status}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid-2">
          <div style={{ padding: '16px', background: '#1a1a1a', borderRadius: '6px' }}>
            <p style={{ color: '#888', margBottom: '8px' }}>Avg Delivery Time</p>
            <p style={{ fontSize: '24px', fontWeight: 'bold', color: '#51cf66' }}>{dashData.kpis.deviations.averageDeliveryTime.value} days</p>
            <p style={{ fontSize: '12px', marginTop: '8px', color: '#888' }}>Target: {dashData.kpis.deviations.averageDeliveryTime.target} days</p>
          </div>
          <div style={{ padding: '16px', background: '#1a1a1a', borderRadius: '6px' }}>
            <p style={{ color: '#888', marginBottom: '8px' }}>Service Level</p>
            <p style={{ fontSize: '24px', fontWeight: 'bold', color: dashData.kpis.deviations.serviceLevel.status === 'good' ? '#51cf66' : '#ff6b6b' }}>{dashData.kpis.deviations.serviceLevel.value}%</p>
            <p style={{ fontSize: '12px', marginTop: '8px', color: '#888' }}>Target: {dashData.kpis.deviations.serviceLevel.target}%</p>
          </div>
          <div style={{ padding: '16px', background: '#1a1a1a', borderRadius: '6px' }}>
            <p style={{ color: '#888', marginBottom: '8px' }}>Cost per Order</p>
            <p style={{ fontSize: '24px', fontWeight: 'bold', color: '#4ecdc4' }}>${dashData.allocation.totalCost}</p>
            <p style={{ fontSize: '12px', marginTop: '8px', color: '#888' }}>Orders: {dashData.orders.assigned}</p>
          </div>
          <div style={{ padding: '16px', background: '#1a1a1a', borderRadius: '6px' }}>
            <p style={{ color: '#888', marginBottom: '8px' }}>CO₂ Emissions</p>
            <p style={{ fontSize: '24px', fontWeight: 'bold', color: '#ffd93d' }}>{dashData.allocation.totalCO2} kg</p>
            <p style={{ fontSize: '12px', marginTop: '8px', color: '#888' }}>Total</p>
          </div>
        </div>
      </div>
      ...
      {dashData.disruptions.totalDisruptions > 0 && (
        <div className="card">
          <div className="card-title">⚠️ Disruptions</div>
